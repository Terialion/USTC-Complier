...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("lu12i.w $t0, 0x3ff00");
    ...
    codegen->append_inst("bcnez $fcc0, label_true");
    codegen->append_inst("addi.w $t0, $zero, 0");
    codegen->append_inst("st.b",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("b label_false");
    codegen->append_inst("label_true", ASMInstruction::Label);
    codegen->append_inst("addi.w $t0, $zero, 1");
    codegen->append_inst("st.b",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("label_false", ASMInstruction::Label);
    ...
    codegen->append_inst("ld.b",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("andi $t0, $t0, 0x1"); // 确保只有最低位有效
    ...
    codegen->append_inst("sltu $t0, $zero, $t1");
    codegen->append_inst("st.b", {"$t0", "$fp", std::to_string(offset_map["%op2"])});
